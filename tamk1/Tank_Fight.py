...
def recv_msg(udp_socket):
    global get_data,m,tank2_posx,tank2_posy
    while True:
        ...
        if recv_data:
            ...
            tank2_posx=int(recv_data[1])
            tank2_posy=int(recv_data[2])
        else:
            m=0
        time.sleep(0.015)
...
def send_msg(udp_socket, target_ip):
    global send_data,n,tank1_posx,tank1_posy
    while True:

        if send_data:
            #print(send_data)
            #print(tank1_posx,tank1_posy)
            temp = str(send_data)+str(tank1_posx)+"|"+str(tank1_posy)
            print("{")
            print(temp)
            print("}")
            flag=udp_socket.sendto(bytes(temp.encode("utf-8")), target_ip)
            print(flag)
            send_data=None
        time.sleep(0.015)
...
def tank1_control_event(settings, tank1, bullets, screen, shoot_sound):
    global n,send_data,tank1_posx,tank1_posy
    ...
    if mykeyslist[pygame.K_LEFT]:  # 如果按键按下，这个值为1
        ...
        n=4
        send_data="left|"
        tank1_posx=tank1.rect.centerx = tank1.rect.centerx - settings.tank_speed
        ...
        print("1上")
        send_data = "up|"
        tank1_posy=tank1.rect.centery = tank1.rect.centery - settings.tank_speed
...
def tank2_control_event(settings, tank2, bullets, screen, sound_shoot):
    global m,get_data,tank2_posx,tank2_posy
    move = pygame.mixer.Sound("music//move.wav")
    move.set_volume(0.1)



    if get_data=="left":  # 如果按键按下，这个值为1
        move.play()
        tank2.img = pygame.image.load("坦克_4.png")
        m=4

        tank2.rect.centerx = tank2_posx
        time.sleep(0.015)
        move.stop()
        if tank2.rect.centerx < 30:
            tank2.rect.centerx = tank2.screen_rect.left
        get_data = None
    elif get_data=="right":  # 如果按键按下，这个值为1
        move.play()
